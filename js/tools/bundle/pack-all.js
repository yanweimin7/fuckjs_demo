...
function parseArgs() {
  const args = { key: KEY_DEFAULT, keyId: 'demo-key', version: '1.0.0', minAppVersion: '1.0.0' };
...
function main() {
  ...
  for (const b of BUNDLES) {
    ...
    if (hasQjc) {
      const jsNewer = hasJs && fs.statSync(jsPath).mtimeMs > fs.statSync(qjcPath).mtimeMs;
      if (jsNewer) {
        console.warn(`WARN ${b.name}: .qjc 落后于 .js，仅打包 .js（请运行 qjsc 或 npm run build 重新编译字节码）`);
      } else {
  ...
  const pubB64 = path.join(__dirname, 'bundle_signing_pub.b64');
  const pubB64Dest = path.join(APP_JS, 'bundle_signing_pub.b64');
  if (fs.existsSync(pubB64)) {
    const srcContent = fs.readFileSync(pubB64);
    const destExists = fs.existsSync(pubB64Dest);
    if (!destExists || !fs.readFileSync(pubB64Dest).equals(srcContent)) {
      fs.writeFileSync(pubB64Dest, srcContent);
      console.log('Copied bundle_signing_pub.b64 -> app/assets/js/');
    }
  }
}
