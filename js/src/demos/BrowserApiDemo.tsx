...
export default function BrowserApiDemo() {
  const [logs, setLogs] = useState<string[]>(['Click a button to test APIs...']);
  ...
  // 2. URL Test
  const testURL = () => {
    const url = new URL('https://user:pass@example.com:8080/p/a/t/h?query=1&name=fuick#hash');
  ...
  const testEvents = () => {
    ...
    const handler = (e: any) => addLog(`Event Received: ${e.type}, detail: ${JSON.stringify(e.detail)}`);
    
    target.addEventListener('custom-event', handler as any);
    target.dispatchEvent(new CustomEvent('custom-event', { detail: { ok: true } }));
          ...
          <Padding padding={16}>
            <Column>
              ...
              <Section title="Console Methods">
                <Row mainAxisAlignment="spaceAround">
                  <Button text="log/info" onTap={() => { console.log('Log test'); console.info('Info test'); addLog('Check native logs'); }} />
                  <Button text="warn/error" onTap={() => { console.warn('Warn test'); console.error('Error test'); addLog('Check native logs'); }} />
                  <Button text="trace" onTap={() => { console.trace(); addLog('Check native logs for stack'); }} />
                </Row>
              </Section>
            </Column>
          </Padding>
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <Column crossAxisAlignment="start" margin={{ bottom: 20 }}>
      <Text text={title} fontSize={16} fontWeight="bold" margin={{ bottom: 10 }} color="#333" />
      {children}
    </Column>
  );
}
